    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                ...
                <p class="text-center">Customer Details Information</p>
                </div>           
                <table width="100%">
                    <thead>
                        <h5>Invoice No:({{ $payment['invoice']['invoice_no']}})</h5>
                    </thead> 
                    <tbody>
                    <tr class="pb-1">
                        <td width="20%"><strong>Name</strong> :{{ $payment['customer']['name'] }}</td>
                        <td width="25%"><strong>MobileNo</strong>:{{ $payment['customer']['mobile'] }}</td>
                    ...
                    <tbody>
                        ...
                        @endphp
                           ...
                           <tr>
                            ...
                            @php
                                $paid_summary = App\Models\PaymentDetails::where('invoice_id', $payment->invoice_id)->get();
                            @endphp
                            @foreach ($paid_summary as $key => $summary)
                            <tr>
                                <td colspan="3" style="text-align: center">{{ $summary->date }}</td>
                                <td colspan="3" style="text-align: center">{{ $summary->current_paid_amount }}</td>
                            </tr>
                            @endforeach
